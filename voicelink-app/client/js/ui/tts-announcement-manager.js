class TTSAnnouncementManager {
    constructor(paSystemManager, builtinEffectsProcessor, audioEngine, spatialAudio) {
    ...
    // Play audio preview using spatial audio processing
    playAudioPreview(audioData) {
        if (this.audioEngine.audioContext && this.spatialAudio) {
            try {
                const source = this.audioEngine.audioContext.createMediaStreamSource(audioData);

                // Route through spatial audio system for binaural processing
                if (this.spatialAudio.masterGain) {
                    source.connect(this.spatialAudio.masterGain);
                    console.log('TTS preview routed through spatial audio system');
                } else {
                    // Fallback to direct connection if spatial audio isn't available
                    source.connect(this.audioEngine.audioContext.destination);
                    console.log('TTS preview using direct audio connection (spatial audio not available)');
                }
            } catch (error) {
                console.error('Error routing TTS preview through spatial audio:', error);
