...
class AccessibleDropdown {
    ...
    handleButtonKeydown(e) {
        switch (e.key) {
            ...
            case 'ArrowDown':
                ...
                // Move to next option even when closed (like native select)
            ...
            case 'ArrowUp':
                ...
                // Move to previous option even when closed (like native select)
            ...
            case 'End':
                ...
                break;
            default:
                // Type-ahead search when button is focused (like native select)
                if (e.key.length === 1 && !e.ctrlKey && !e.altKey && !e.metaKey) {
                    e.preventDefault();
                    this.typeAheadSearchAndSelect(e.key);
                }
                break;
        }
    }
    ...
    open() {
        ...
        // Set initial focus on the currently selected option
    ...
    focusNextOption() {
        ...
        const optionCount = this.originalSelect.options.length;
        let nextIndex;

        if (currentFocused) {
            nextIndex = parseInt(currentFocused.getAttribute('data-index'), 10) + 1;
        } else {
            // Start from selected index
        ...
        // If dropdown is closed, also select the option (like native select)
        if (!this.isOpen) {
    ...
    typeAheadSearch(char) {
        const index = this.findOptionByChar(char);
        if (index !== -1) {
            this.focusOption(index);
        }
    }

    typeAheadSearchAndSelect(char) {
        const index = this.findOptionByChar(char);
        if (index !== -1) {
            this.selectOption(index);
        }
    ...
    findOptionByChar(char) {
        const searchChar = char.toLowerCase();
        const options = this.listbox.querySelectorAll('.dropdown-option');

        // Build search string for multi-character type-ahead
        clearTimeout(this.typeAheadTimeout);
        this.typeAheadBuffer = (this.typeAheadBuffer || '') + searchChar;
        this.typeAheadTimeout = setTimeout(() => {
            this.typeAheadBuffer = '';
        }, 500);

        // Find first option starting with the buffer after current position
        const currentFocused = this.listbox.querySelector('.focused');
        let startIndex = 0;

        if (currentFocused) {
            startIndex = parseInt(currentFocused.getAttribute('data-index'), 10);
            // If buffer is single char, start from next; if multi-char, start from current
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            if (optionText.startsWith(this.typeAheadBuffer)) {
                return index;
            }
        }

        // If multi-char didn't match, try just the last character
        if (this.typeAheadBuffer.length > 1) {
            for (let i = 0; i < options.length; i++) {
                const index = (startIndex + i) % options.length;
                const optionText = options[index].textContent.toLowerCase();
                ...
                if (optionText.startsWith(searchChar)) {
                    this.typeAheadBuffer = searchChar;
                    return index;
                }
            }
        }
