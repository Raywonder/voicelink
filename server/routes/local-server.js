    ...
    setupSocketHandlers() {
        this.io.on('connection', (socket) => {
            ...
            socket.on('join-room', (data) => {
                ...
                // If user already in room, don't count them against capacity
                ...
                const desiredName = userName || username || authUser?.displayName || authUser?.username || `User ${socket.id.slice(0, 8)}`;

                // If user already exists in this room, update and re-send state
                if (existingUser) {
