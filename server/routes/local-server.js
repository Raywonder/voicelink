...
class VoiceLinkLocalServer {
    ...
    setupSocketHandlers() {
        this.io.on('connection', (socket) => {
            ...
            // Audio relay support - enables server-relayed audio when P2P fails
            socket.on('enable-audio-relay', (data) => {
                const user = this.users.get(socket.id);
                if (user) {
                    user.audioRelayEnabled = true;
                    user.relaySettings = {
                        sampleRate: data.sampleRate || 48000,
                        channels: data.channels || 2
                    };
                    console.log(`[Audio] Relay enabled for user ${user.name} (${socket.id})`);

                    // Notify room that this user is in relay mode
                    socket.to(user.roomId).emit('relay-status', {
                        userId: socket.id,
                        active: true,
            ...
            socket.on('audio-data', (data) => {
                const user = this.users.get(socket.id);
                if (user && user.roomId) {
                    // Relay audio to all other users in the same room
                    socket.to(user.roomId).emit('relayed-audio', {
                        userId: socket.id,
            ...
            socket.on('p2p-connection-failed', (data) => {
                const user = this.users.get(socket.id);
                if (user) {
                    console.log(`[Audio] P2P failed for ${socket.id} to ${data.targetUserId}: ${data.reason}`);
                    // Notify the target user to switch to relay mode
                    this.io.to(data.targetUserId).emit('p2p-fallback-needed', {
                        userId: socket.id,
                        reason: data.reason
                    });
                }
