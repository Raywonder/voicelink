...
class MastodonBotManager {
    constructor(server) {
        ...
        this.commands = {
            ...
            'devices': this.handleDevicesCommand.bind(this),
            'revoke': this.handleRevokeCommand.bind(this)
        };
    ...
    async handleHelpCommand(userHandle, args, replyToId, bot) {
        ...
        let message = `@${userHandle.replace('@', '')} 🎙️ VoiceLink Bot Commands\n\n` +
            ...
            `!revoke <device> - Revoke device access\n` +
            `!revoke all - Revoke all devices`;
    ...
    /**
     * Handle devices command - list user's linked devices
     */
    async handleDevicesCommand(userHandle, args, replyToId, bot) {
        const devices = this.getUserDevices(userHandle);

        if (devices.length === 0) {
            return this.postToInstance(bot.instance, bot.accessToken,
                `@${userHandle.replace('@', '')} 📱 You have no linked devices.\n\n` +
                `To link a device, use VoiceLink app and sign in with your Mastodon account.`, {
                visibility: 'direct',
                inReplyToId: replyToId
            });
        }

        let message = `@${userHandle.replace('@', '')} 📱 Your Linked Devices:\n\n`;

        devices.forEach((device, index) => {
            const status = device.isRevoked ? '🔴 Revoked' : '🟢 Active';
            const authIcon = device.authMethod === 'mastodon' ? '🐘' :
                             device.authMethod === 'email' ? '📧' : '🔑';
            message += `${index + 1}. ${authIcon} ${device.deviceName}\n`;
            message += `   ${status} | Linked: ${this.formatDate(device.linkedAt)}\n`;
        });

        message += `\nTo revoke a device: !revoke <device-name>\n`;
        message += `To revoke all: !revoke all`;
    ...
    /**
     * Handle revoke command - revoke device access
     */
    async handleRevokeCommand(userHandle, args, replyToId, bot) {
        if (args.length === 0) {
            return this.postToInstance(bot.instance, bot.accessToken,
                `@${userHandle.replace('@', '')} Usage:\n` +
                `!revoke <device-name> - Revoke specific device\n` +
                `!revoke all - Revoke all your devices\n\n` +
                `Use !devices to see your linked devices first.`, {
                visibility: 'direct',
                inReplyToId: replyToId
            });
        }
        ...
        if (devices.length === 0) {
            return this.postToInstance(bot.instance, bot.accessToken,
                `@${userHandle.replace('@', '')} ❌ You have no linked devices to revoke.`, {
                visibility: 'direct',
                inReplyToId: replyToId
            });
        }
        ...
        if (target === 'all') {
            ...
            for (const device of devices) {
                if (!device.isRevoked) {
                    this.revokeDevice(device.id, 'Revoked via Mastodon command');
                    revokedCount++;
                }
            }

            return this.postToInstance(bot.instance, bot.accessToken,
                `@${userHandle.replace('@', '')} ✅ Revoked access for ${revokedCount} device(s).\n\n` +
                `All your devices have been disconnected from VoiceLink.`, {
                visibility: 'direct',
                inReplyToId: replyToId
            });
        }
        ...
        const device = devices.find(d =>
            d.deviceName.toLowerCase().includes(target) ||
            d.id.toLowerCase() === target
        );

        if (!device) {
            return this.postToInstance(bot.instance, bot.accessToken,
                `@${userHandle.replace('@', '')} ❌ Device "${args.join(' ')}" not found.\n\n` +
                `Use !devices to see your linked devices.`, {
                visibility: 'direct',
                inReplyToId: replyToId
            });
        }
        ...
        if (device.isRevoked) {
            return this.postToInstance(bot.instance, bot.accessToken,
                `@${userHandle.replace('@', '')} ℹ️ Device "${device.deviceName}" is already revoked.`, {
                visibility: 'direct',
                inReplyToId: replyToId
            });
        }
        ...
        return this.postToInstance(bot.instance, bot.accessToken,
            `@${userHandle.replace('@', '')} ✅ Device "${device.deviceName}" has been revoked.\n\n` +
            `The device will be disconnected from VoiceLink.`, {
            visibility: 'direct',
            inReplyToId: replyToId
        });
    }
    ...
    /**
     * Get devices linked to a Mastodon user
     */
    getUserDevices(userHandle) {
        const linkedDevices = this.server?.linkedDevices || new Map();
        const normalizedHandle = userHandle.toLowerCase().replace(/^@/, '');

        const devices = [];
        linkedDevices.forEach((device, id) => {
            // Check if device is linked via Mastodon with matching handle
            if (device.authMethod === 'mastodon') {
                const deviceHandle = (device.authUsername || '').toLowerCase().replace(/^@/, '');
                if (deviceHandle === normalizedHandle ||
                    deviceHandle.includes(normalizedHandle.split('@')[0])) {
    ...
    revokeDevice(deviceId, reason) {
        const linkedDevices = this.server?.linkedDevices;
        if (!linkedDevices) return false;
        ...
        if (this.server?.io) {
            this.server.io.emit('access-revoked', {
                deviceId,
                reason: reason || 'Access revoked'
            });
        }

        console.log(`Device ${deviceId} (${device.deviceName}) revoked via Mastodon command`);
        return true;
    }
