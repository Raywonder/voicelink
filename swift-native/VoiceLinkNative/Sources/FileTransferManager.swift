...
class FileTransferManager: ObservableObject {
    ...
    private let transferGuideSeenKey = "hasSeenFileTransferGuide"
    ...
    struct FileTransferSettings: Codable, Equatable {
        ...
        var autoOpenSavedFiles: Bool = true
    ...
    func getSettingsForAPI() -> [String: Any] {
        return [
            ...
            "autoOpenSavedFiles": settings.autoOpenSavedFiles
        ]
    }
    ...
    func applySettingsFromAPI(_ data: [String: Any]) {
        ...
        if let autoOpenSavedFiles = data["autoOpenSavedFiles"] as? Bool {
            settings.autoOpenSavedFiles = autoOpenSavedFiles
        }
        saveSettings()
    }
    ...
    func showFilePicker(completion: @escaping (URL?) -> Void) {
        if !UserDefaults.standard.bool(forKey: transferGuideSeenKey) {
            presentTransferGuide { [weak self] proceed in
                guard let self = self, proceed else {
                    completion(nil)
                    return
                }
                self.presentOpenPanel(completion: completion)
            }
            return
        }

        presentOpenPanel(completion: completion)
    }

    private func presentTransferGuide(completion: @escaping (Bool) -> Void) {
        let alert = NSAlert()
        alert.messageText = "Before You Share Files"
        alert.informativeText = """
        VoiceLink transfers a copy of the file you choose.

        What to expect:
        • Sent files stay in their original location.
        • Received files stay in Transfers until you choose Save.
        • Saved files are copied to your Downloads folder.
        • Only continue if you trust the person and the file.

        This guide is shown once.
        """
        ...
        UserDefaults.standard.set(true, forKey: transferGuideSeenKey)
    ...
    func saveToDownloads(_ transfer: FileTransfer) {
        ...
        do {
            ...
            if settings.autoOpenSavedFiles {
        ...
        VStack(spacing: 0) {
            ...
            HStack {
                Toggle(
                    ...
                    isOn: Binding(
                        get: { transferManager.settings.autoOpenSavedFiles },
                        set: { newValue in
                            var updated = transferManager.settings
                            updated.autoOpenSavedFiles = newValue
                            transferManager.updateSettings(updated)
                        }
                    )
                )
