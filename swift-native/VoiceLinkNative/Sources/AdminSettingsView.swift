        ...
        VStack(alignment: .leading, spacing: 20) {
            AdminHelpSection(
                ...
                steps: [
                    ...
                    "Use the Bots tab to enable cross-agent delegation, moderation watches for guests or spam, and server-side defaults shared with Thrive/OpenLink integrations."
                ],
                docs: [
            ...
            if var config = settings {
                ...
                case .whmcs:
                    ...
                    if config.whmcsEnabled {
                        ...
                        ConfigTextField(
                            ...
                            text: Binding(
                                get: { config.whmcsPortalUrl ?? "" },
                                set: { config.whmcsPortalUrl = $0.isEmpty ? nil : $0; settings = config }
                            )
                        )
                        ...
                        ConfigTextField(
                            ...
                            text: Binding(
                                get: { config.whmcsAdminUrl ?? "" },
                                set: { config.whmcsAdminUrl = $0.isEmpty ? nil : $0; settings = config }
                            )
                        )

                        ConfigToggle(label: "Allow Client Portal Launch", helpText: "Permit connected VoiceLink desktop clients to open the configured WHMCS client portal from this server.", isOn: Binding(
                            get: { config.whmcsAllowClientPortalLaunch },
                        ...
                        ConfigTextField(
                            ...
                            text: Binding(
                                ...
                                set: { config.whmcsEndpointAccessMode = $0.isEmpty ? "managed" : $0; settings = config }
                            )
                        )
                        ...
                        ConfigTextField(
                            ...
                            text: Binding(
                                get: { config.whmcsManagedEndpointPolicy },
                                set: { config.whmcsManagedEndpointPolicy = $0.isEmpty ? "server-linked" : $0; settings = config }
                            )
                        )
                    }

                case .openlink:
                    SectionHeader(title: "OpenLink Voice Fallback")

                    ConfigToggle(label: "Enable OpenLink Module", helpText: "Turn on OpenLink-specific backend handling on this server. Leave this off until OpenLink fallback voice rooms should be active here.", isOn: Binding(
                        get: { config.openlinkEnabled },
                        set: { config.openlinkEnabled = $0; settings = config }
                    ))

                    if config.openlinkEnabled {
                        ConfigToggle(label: "Enable Voice Fallback Rooms", helpText: "Create hidden VoiceLink fallback rooms for OpenLink sessions when direct P2P voice is unavailable.", isOn: Binding(
                            get: { config.openlinkVoiceFallbackRoomsEnabled },
                            set: { config.openlinkVoiceFallbackRoomsEnabled = $0; settings = config }
                        ))

                        ConfigToggle(label: "Require Admin Approval For Entry", helpText: "Keep OpenLink fallback rooms locked so admins can see them in overview but must be explicitly approved before entering.", isOn: Binding(
                            get: { config.openlinkRequireAdminApprovalForEntry },
                            set: { config.openlinkRequireAdminApprovalForEntry = $0; settings = config }
                        ))

                        ConfigToggle(label: "Allow Admin Override", helpText: "Permit an admin override window for OpenLink rooms when staff must enter without waiting for normal approval flow.", isOn: Binding(
                            get: { config.openlinkAllowAdminOverride },
                            set: { config.openlinkAllowAdminOverride = $0; settings = config }
                        ))

                        ConfigToggle(label: "Notify Before Admin Override", helpText: "Send VoiceLink admin notifications before an override window is used for an OpenLink room.", isOn: Binding(
                            get: { config.openlinkNotifyBeforeAdminOverride },
                            set: { config.openlinkNotifyBeforeAdminOverride = $0; settings = config }
                        ))

                        ConfigToggle(label: "Show Active Rooms In Admin Overview", helpText: "Show active hidden OpenLink rooms in admin room overview with domain, source, and fallback state while keeping them hidden from normal room lists.", isOn: Binding(
                            get: { config.openlinkShowActiveRoomsInAdminOverview },
                            set: { config.openlinkShowActiveRoomsInAdminOverview = $0; settings = config }
                        ))

                        ConfigNumberField(
                            label: "Room Duration (minutes)",
                            helpText: "How long OpenLink fallback rooms should be kept before automatic expiry.",
                            value: Binding(
                                get: { config.openlinkRoomDurationMinutes },
                                set: { config.openlinkRoomDurationMinutes = max(5, $0); settings = config }
                            )
                        )

                        ConfigNumberField(
                            label: "Admin Override Window (seconds)",
                            helpText: "How long an armed admin override should allow staff entry into a locked OpenLink room.",
                            value: Binding(
                                get: { config.openlinkOverrideWindowSeconds },
                                set: { config.openlinkOverrideWindowSeconds = max(30, $0); settings = config }
                            )
                        )

                        ConfigTextField(
                            label: "Default OpenLink Domain",
                            placeholder: "openlink.tappedin.fm",
                            helpText: "Default generated OpenLink domain shown in admin overview when a session has not supplied a more specific domain.",
                            text: Binding(
                                get: { config.openlinkDefaultDomain },
                                set: { config.openlinkDefaultDomain = $0.isEmpty ? "openlink.tappedin.fm" : $0; settings = config }
                            )
                        )
                    }
                ...
                case .bots:
                    ...
                    ConfigToggle(
                        ...
                        isOn: Binding(
                            get: { config.botsEnabled },
                            set: { config.botsEnabled = $0; settings = config }
                        )
                    )
                    ...
                    ConfigToggle(
                        ...
                        isOn: Binding(
                            get: { config.botMeshEnabled },
                            set: { config.botMeshEnabled = $0; settings = config }
                        )
                    )
                    ...
                    ConfigToggle(
                        ...
                        isOn: Binding(
                            get: { config.botModerationEnabled },
                            set: { config.botModerationEnabled = $0; settings = config }
                        )
                    )
                    ...
                    if config.botModerationEnabled {
                        ...
                        ConfigToggle(label: "Watch Guest Logins", helpText: "Emit events when guest-style accounts sign in.", isOn: Binding(
                            get: { config.botWatchGuestLogins },
                            set: { config.botWatchGuestLogins = $0; settings = config }
                        ))

                        ConfigToggle(label: "Watch Room Messages", helpText: "Reserve room-level bot moderation policy for VoiceLink rooms and spam scanning.", isOn: Binding(
                            get: { config.botWatchRoomMessages },
                            set: { config.botWatchRoomMessages = $0; settings = config }
                        ))

                        ConfigToggle(label: "Watch Direct Messages", helpText: "Emit moderation events for direct or support-side message traffic.", isOn: Binding(
                            get: { config.botWatchDirectMessages },
                            set: { config.botWatchDirectMessages = $0; settings = config }
                        ))

                        ConfigToggle(label: "Watch File Offers", helpText: "Emit moderation events when attachments or file relay offers are made.", isOn: Binding(
                            get: { config.botWatchFileOffers },
                            set: { config.botWatchFileOffers = $0; settings = config }
                        ))

                        SectionHeader(title: "Notifications and Relay")

                        ConfigToggle(label: "Notify Admin Staff", helpText: "Send moderation output to admin/staff workflows by default.", isOn: Binding(
                            get: { config.botNotifyAdmins },
                            set: { config.botNotifyAdmins = $0; settings = config }
                        ))

                        ConfigToggle(label: "Notify Support Rooms", helpText: "Allow bot moderation summaries to surface in support-focused flows when enabled.", isOn: Binding(
                            get: { config.botNotifySupportRooms },
                            set: { config.botNotifySupportRooms = $0; settings = config }
                        ))
                    }
                    ...
                    ConfigToggle(
                        ...
                        isOn: Binding(
                            get: { config.botAllowFileRelay },
                            set: { config.botAllowFileRelay = $0; settings = config }
                        )
                    )
                    ...
                    ConfigTextField(
                        ...
                        text: Binding(
                            get: { config.botDefaultDelegateBot },
                            set: { config.botDefaultDelegateBot = $0.isEmpty ? "codex-bot" : $0; settings = config }
                        )
                    )
                    ...
                    ConfigTextField(
                        ...
                        text: Binding(
                            get: { config.botPreferredBackends.joined(separator: ", ") },
                            set: {
                                config.botPreferredBackends = $0
                                    .split(separator: ",")
                                    .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
                                    .filter { !$0.isEmpty }
                                settings = config
                    ...
                    ConfigTextField(
                        ...
                        text: Binding(
                            get: { config.botTempDirectory ?? "" },
                            set: { config.botTempDirectory = $0.isEmpty ? nil : $0; settings = config }
                        )
                    )
                    ...
                    ConfigNumberField(
                        ...
                        value: Binding(
                            get: { config.botMaxRelayFileSize },
                            set: { config.botMaxRelayFileSize = max(1024, $0); settings = config }
                        )
                    )
                }
